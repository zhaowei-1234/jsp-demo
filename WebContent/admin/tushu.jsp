...
try{
	...
	rs=stmt.executeQuery(sql4Num);
	rs.next();
	//System.out.println("getINt()"+rs.getInt(1));
	totalrecord=rs.getInt(1);
	
	rs=stmt.executeQuery(sql4);// 发送sql语句返回结果集
	...
	for(int i=0;i<5;i++){
		//id=rs.getInt(1);
		ids = rs.getString(1);
		out.print("<tr><td align='center' valign='middle'>"+ids+"</td><td align='center' valign='middle'>"+rs.getFloat(2)+"</td><td align='center' valign='middle'><img src='"+rs.getString(3)+"' width='180' height='170' style='display: inline-block;'/>"+"</td><td align='center' valign='middle'><a href=modify.jsp?cat=图书&ids="+ids+">修改</a></td><td align='center' valign='middle'><a href=delete.jsp?cat=图书&ids="+ids+">删除</a> </td></tr>");
	    if(!rs.next())break;
	}
	out.print("</table>");
...
if(p==1){
	%>
 <a href=tushu.jsp?p=<%=p+1 %>>下一页</a>   <a href=tushu.jsp?p=<%=totalpage%>>尾页</a> 
<%}
else if(p==totalpage){
	%>	
	<a href=tushu.jsp>首页</a>   <a href=tushu.jsp?p=<%=p-1 %>>上一页</a> 
